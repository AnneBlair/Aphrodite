...
/// BarChartRenderer
open class ABarRenderer: ALineBubble
{
    ...
    open weak var dataProvider: ABarDataProvider?
    
    public init(dataProvider: ABarDataProvider?, animator: Animator?, viewPortHandler: PortHandler?)
    {
    ...
    open override func initBuffers()
    {
        if let barData = dataProvider?.barData
        {
            ...
            {
                let set = barData.dataSets[i] as! AIBarDataSet
    ...
    fileprivate func prepareBuffer(dataSet: AIBarDataSet, index: Int)
    {
        guard
        ...
        {
            guard let e = dataSet.entryForIndex(i) as? ABarDataEntry else { continue }
    ...
    open override func drawData(context: CGContext)
    {
        ...
        for i in 0 ..< barData.dataSetCount
        {
            ...
            if set.isVisible
            {
                if !(set is AIBarDataSet)
                {
                    fatalError("Datasets for BarChartRenderer must conform to AIBarDataSet")
                }
                
                drawDataSet(context: context, dataSet: set as! AIBarDataSet, index: i)
            }
        }
    }
    ...
    open func drawDataSet(context: CGContext, dataSet: AIBarDataSet, index: Int)
    {
        guard
            let dataProvider = dataProvider,
    ...
    open override func drawValues(context: CGContext)
    {
        ...
        if isDrawingValuesAllowed(dataProvider: dataProvider)
        {
            ...
            {
                guard let dataSet = dataSets[dataSetIndex] as? AIBarDataSet else { continue }
                ...
                if !dataSet.isStacked
                {
                    ...
                    {
                        guard let e = dataSet.entryForIndex(j) as? ABarDataEntry else { continue }
                ...
                else
                {
                    ...
                    {
                        guard let e = dataSet.entryForIndex(index) as? ABarDataEntry else { continue }
    ...
    open func drawValue(context: CGContext, value: String, xPos: CGFloat, yPos: CGFloat, font: Font, align: NSTextAlignment, color: Color)
    {
        AUtils.drawText(context: context, text: value, point: CGPoint(x: xPos, y: yPos), align: align, attributes: [NSAttributedStringKey.font: font, NSAttributedStringKey.foregroundColor: color])
    }
    ...
    open override func drawHighlighted(context: CGContext, indices: [Highlight])
    {
        ...
        for high in indices
        {
            ...
            if let e = set.entryForXValue(high.x, closestToY: high.y) as? ABarDataEntry
            {
