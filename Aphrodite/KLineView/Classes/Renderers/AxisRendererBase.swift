...
open class AxisRendererBase: Renderer
{
    ...
    public init(viewPortHandler: PortHandler?, transformer: Transformer?, axis: AxisBase?)
    {
    ...
    open func computeAxisValues(min: Double, max: Double)
    {
        ...
        var interval = AUtils.roundToNextSignificant(number: Double(rawInterval))
        ...
        // Normalize interval
        let intervalMagnitude = AUtils.roundToNextSignificant(number: pow(10.0, Double(Int(log10(interval)))))
        ...
        else
        {
            ...
            let last = interval == 0.0 ? 0.0 : AUtils.nextUp(floor(yMax / interval) * interval)
