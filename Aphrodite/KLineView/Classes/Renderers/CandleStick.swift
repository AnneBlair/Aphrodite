...
open class  CandleStick: ALineCandleRadar {
    
    open weak var dataProvider: ACandleChartData?
    
    public init(dataProvider: ACandleChartData?, animator: Animator?, viewPortHandler: PortHandler?)
    {
    ...
    open override func drawData(context: CGContext)
    {
        ...
        for set in candleData.dataSets as! [ACandleDataSet]
        {
            if set.isVisible
            {
    ...
    open func drawDataSet(context: CGContext, dataSet: ACandleDataSet)
    {
        guard let
        ...
        {
            // get the entry
            guard let e = dataSet.entryForIndex(j) as? CandleDataEntry else { continue }
    ...
    open override func drawValues(context: CGContext)
    {
        ...
        if isDrawingValuesAllowed(dataProvider: dataProvider)
        {
            ...
            {
                guard let dataSet = dataSets[i] as? LineBubbleDataSet
                    else { continue }
                ...
                for j in stride(from: _xBounds.min, through: _xBounds.range + _xBounds.min, by: 1)
                {
                    guard let e = dataSet.entryForIndex(j) as? CandleDataEntry else { break }
                    ...
                    {
                        AUtils.drawImage(context: context,
                                             image: icon,
    ...
    open override func drawHighlighted(context: CGContext, indices: [Highlight])
    {
        ...
        {
            guard
                let set = candleData.getDataSetByIndex(high.dataSetIndex) as? ACandleDataSet,
                set.isHighlightEnabled
                else { continue }
            
            guard let e = set.entryForXValue(high.x, closestToY: high.y) as? CandleDataEntry else { continue }
