...
open class XAxisRenderer: AxisRendererBase {
    ...
    public init(viewPortHandler: PortHandler?, xAxis: XAxis?, transformer: Transformer?) {
    ...
    open func computeSize() {
        ...
        let labelRotatedSize = AUtils.sizeOfRotatedRectangle(labelSize, degrees: xAxis.labelRotationAngle)
    ...
    open func drawLabels(context: CGContext, pos: CGFloat, anchor: CGPoint) {
        ...
        let labelAttrs: [NSAttributedStringKey: NSObject] = [NSAttributedStringKey.font: xAxis.labelFont,
                          NSAttributedStringKey.foregroundColor: xAxis.labelTextColor,
                          NSAttributedStringKey.paragraphStyle: paraStyle]
        let labelRotationAngleRadians = xAxis.labelRotationAngle * AUtils.Math.FDEG2RAD
    ...
    open func renderLimitLineLine(context: CGContext, limitLine: ALimitLine, position: CGPoint) {
        guard
            let viewPortHandler = self.viewPortHandler
            else { return }
    ...
    open func renderLimitLineLabel(context: CGContext, limitLine: ALimitLine, position: CGPoint, yOffset: CGFloat)
    {
        guard
            let viewPortHandler = self.viewPortHandler
            else { return }
        ...
        if limitLine.drawLabelEnabled && label.characters.count > 0 {
            ...
            if limitLine.labelPosition == .rightTop {
                AUtils.drawText(context: context,
                                    text: label,
                                    point: CGPoint(
