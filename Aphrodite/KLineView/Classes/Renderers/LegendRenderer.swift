...
@objc(ALegendRenderer)
open class LegendRenderer: Renderer
{
    /// the legend object this renderer renders
    open var legend: Legend?
    
    public init(viewPortHandler: PortHandler?, legend: Legend?)
    {
    ...
    open func computeLegend(data: AData)
    {
        guard
            let legend = legend,
        ...
        if !legend.isLegendCustom
        {
            ...
            for i in 0..<data.dataSetCount
            {
                ...
                if dataSet is AIBarDataSet &&
                    (dataSet as! AIBarDataSet).isStacked
                {
                    let bds = dataSet as! AIBarDataSet
