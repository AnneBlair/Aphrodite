...
open class AxisBase: ComponentBase {
    ...
    fileprivate var _axisValueFormatter: AxisValue?
    ...
    fileprivate var _limitLines = [ALimitLine]()
    ...
    open var valueFormatter: AxisValue? {
        get {
            if _axisValueFormatter == nil ||
                (_axisValueFormatter is DefaultAxisValue &&
                    (_axisValueFormatter as! DefaultAxisValue).hasAutoDecimals &&
                    (_axisValueFormatter as! DefaultAxisValue).decimals != decimals) {
                _axisValueFormatter = DefaultAxisValue(decimals: decimals)
            }
            
            return _axisValueFormatter
        }
        set {
            _axisValueFormatter = newValue ?? DefaultAxisValue(decimals: decimals)
        }
    }
    ...
    /// Adds a new ALimitLine to this axis.
    open func addLimitLine(_ line: ALimitLine) {
        _limitLines.append(line)
    }
    
    /// Removes the specified ALimitLine from the axis.
    open func removeLimitLine(_ line: ALimitLine) {
    ...
    open var limitLines : [ALimitLine] {
        return _limitLines
    }
